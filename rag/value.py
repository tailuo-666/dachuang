...
"""
    ...
    # 本地 embedding 模型路径（与 rag_system.py 保持一致）
    local_embedding_path: str = "/root/.cache/modelscope/hub/models/BAAI/bge-m3"
    ...
    def __init__(self, **kwargs):
        ...
        # 初始化 LLM (用于LLM评估策略，延迟加载)
        # 如需使用 LLM 策略，可在运行时初始化
    ...
    def _evaluate_with_bert(self, query: str, docs: List[str]) -> Dict:
        ...
        if not self.embedding_model:
            ...
            if os.path.exists(self.local_embedding_path):
                logger.info(f"使用本地 BGE-m3 模型: {self.local_embedding_path}")
                ...
                logger.warning(f"本地模型不存在: {self.local_embedding_path}，使用在线模型")
                self.embedding_model = HuggingFaceEmbeddings(
